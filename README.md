...
* `plural` &mdash; объект со свойствами-падежами для текста во множественном числе.
...
Метод `russian->spell(int $number, string $unit)` решает задачу получения прописи числа
...
Украинский вариант склонения &mdash; метод `ukrainian->declension(string $phrase, string ...$flags)`:
...
Метод `ukrainian->spell(int $number, string $unit)` решает задачу получения прописи числа
(одна тисяча сто двадцять п'ять) и согласование единицы измерения с предшествующем числом (один рубль, два рубля, п'ять
рублів):
...
* `instrumental` &mdash; текст в творительном падеже;
* `plural` &mdash; возвращает объект со свойствами-падежами для текста во множественном числе.
...
метод `russian->userDict->remove(string $nominativeForm)`:
